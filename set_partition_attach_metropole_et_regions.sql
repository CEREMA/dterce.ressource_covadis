...
IF partition_sans_millesime is true
	THEN
		req := '
			--DROP TABLE IF EXISTS ' || racine_table || '_000 ;
			CREATE TABLE ' || racine_table || '_000 (LIKE ' || racine_table || '_001' || millesime || ' INCLUDING COMMENTS) PARTITION BY LIST (code_dep);
		';
	ELSE
		req := '
...
RETURN current_date || ' - ' || racine_table || '_000' || millesime || ' reste visible';
END; 
$BODY$;
